...
function register({bot,ownerId=config.ownerId,isClone=false,cloneId=null}){
 ...
 bot.onText(/^\/clone$/,async msg=>{
  ...
  if((await clones.credits(msg.from.id))<1)return bot.sendMessage(msg.chat.id,`❌ Clone kreditingiz yo‘q. /shop orqali ${(await economyPrices.get()).clone_price} Stars ga oling.`);
 ...
 bot.onText(/^\/owner$/,async msg=>{
  ...
  return bot.sendMessage(msg.chat.id,await ownerSvc.render(owner()) + (isClone?'\n\n🧬 CLONE OWNER\nBu panel faqat shu clone uchun admin va sozlamalarni boshqaradi.':''),{reply_markup:kb});
 });

 bot.onText(/^\/addadmin\s+(\d+)$/,async msg=>{
  if(!isOwner(msg.from.id,owner()))return;
  const id=Number(msg.match[1]); await admins.ensure(id,owner()); permissionDrafts.set(`${owner()}:${id}`,new Set());
 ...
 bot.on('message',async msg=>{
  ...
  try{
   if(state.type==='channel'){
    ...
    await groups.addRequiredChannel(parts[0],parts[1],parts.slice(2).join('|'),cloneId);
   ...
   if(state.type==='admin'){
    ...
    await admins.ensure(id,owner()); permissionDrafts.set(`${owner()}:${id}`,new Set()); pending.delete(keyFor(msg.from.id));
   ...
   if(state.type==='clone_token'){
    ...
    if((await clones.byOwner(owner())).some(x=>String(x.bot_id)===String(info.id)))throw new Error('Bu bot allaqachon clone sifatida ulangan.');
    if(!(await clones.consumeCredit(state.creatorId||msg.from.id)))throw new Error('Clone krediti topilmadi.');
    const row=await clones.create(state.creatorId||msg.from.id,secret.encrypt(token),info.id,info.username,info.first_name);
    pending.delete(keyFor(msg.from.id));
    ...
    try{await runner.start(row);}catch(e){await clones.setStatus(row.id,'error',e.message);await bot.sendMessage(msg.chat.id,'⚠️ Clone saqlandi, ammo ishga tushmadi. Status owner panelda ko‘rinadi.');}
   }
  }catch(e){return bot.sendMessage(msg.chat.id,`❌ ${e.message}`);}
 });

 bot.onText(/^\/setprice\s+(money|coin|diamond|clone_price)\s+(\d+)$/,async msg=>{if(isClone||!isOwner(msg.from.id,owner()))return;try{const c=await economyPrices.set(msg.match[1],msg.match[2]);await bot.sendMessage(msg.chat.id,`✅ ${msg.match[1]} narxi ${c[msg.match[1]]} ⭐ qilib o‘zgartirildi.`);}catch(e){await bot.sendMessage(msg.chat.id,'❌ '+e.message);}});

 bot.on('callback_query',async q=>{
  if(!isOwner(q.from.id,owner()))return;
  const d=q.data||'';
  try{
   if(d==='owner:panel'){const t=await ownerSvc.render(owner());await bot.answerCallbackQuery(q.id);return bot.sendMessage(q.message.chat.id,t + (isClone?'\n\n🧬 CLONE OWNER':''),{reply_markup:isClone?cloneOwnerPanel():panel()});}
   if(d==='owner:stats'){
    const s=await stats.totals(),cs=await clones.globalStats(); await bot.answerCallbackQuery(q.id);
   ...
   if(d==='owner:channels'){
    const rows=await groups.requiredChannels(cloneId); await bot.answerCallbackQuery(q.id);
   ...
   if(d.startsWith('channel:remove:')){const id=decodeURIComponent(d.split(':').slice(2).join(':'));await groups.removeRequiredChannel(id,cloneId);await bot.answerCallbackQuery(q.id,{text:'Kanal olib tashlandi'});return bot.sendMessage(q.message.chat.id,'🗑 Majburiy kanal olib tashlandi.');}
   if(d==='owner:admins'){
    await bot.answerCallbackQuery(q.id);const rows=await admins.all(owner());
    return bot.sendMessage(q.message.chat.id,`🛡 ADMINS\n${rows.map(a=>`• ${a.user_id}: ${a.enabled?'ACTIVE':'DISABLED'} | ${a.permissions.length?a.permissions.join(', '):'hech biri'}`).join('\n')||'Admin yo‘q.'}`,{reply_markup:await adminsPanel(owner())});
   }
   ...
   if(d.startsWith('admin:view:')){
    const id=Number(d.split(':')[2]),a=await admins.get(id,owner());if(!a)return bot.answerCallbackQuery(q.id,{text:'Admin topilmadi'});
    permissionDrafts.set(`${owner()}:${id}`,new Set(a.permissions));await bot.answerCallbackQuery(q.id);return bot.sendMessage(q.message.chat.id,await adminPerm.text(id,owner()),{reply_markup:permissions(id,a.permissions)});
   }
   ...
   if(d.startsWith('admin:save:')){
    ...
    for(const p of admins.PERMISSIONS)await admins.setPermission(id,owner(),p,set.has(p));
    await bot.answerCallbackQuery(q.id,{text:'Saqlanildi'});return bot.sendMessage(q.message.chat.id,await adminPerm.text(id,owner()),{reply_markup:await adminsPanel(owner())});
   }
   if(d.startsWith('admin:remove:')){const id=Number(d.split(':')[2]);await admins.remove(id,owner());permissionDrafts.delete(`${owner()}:${id}`);await bot.answerCallbackQuery(q.id,{text:'Admin o‘chirildi'});return bot.sendMessage(q.message.chat.id,'🗑 Admin o‘chirildi.',{reply_markup:await adminsPanel(owner())});}
   ...
   if(d==='owner:currency'){await bot.answerCallbackQuery(q.id);const c=await economyPrices.get();return bot.sendMessage(q.message.chat.id,`💱 CURRENCY PRICES\n\n💵 Money: ${c.money} ⭐\n🪙 Coin: ${c.coin} ⭐\n💎 Diamond: ${c.diamond} ⭐\n\nOwner o‘zgartirishi: /setprice money 10 | /setprice coin 100 | /setprice diamond 50 | /setprice clone_price 100`);}
   if(d==='owner:settings'){await bot.answerCallbackQuery(q.id);const c=await economyPrices.get();return bot.sendMessage(q.message.chat.id,`⚙️ SETTINGS\nPlayers: ${config.minPlayers}-${config.maxPlayers}\nClone price: ${c.clone_price} Stars\nCurrency: money=${c.money}, diamond=${c.diamond}, coin=${c.coin}`);}
   if(d==='owner:clones'){
    if(!canManageClones()){await bot.answerCallbackQuery(q.id,{text:'Clone owner bu bo‘limga kira olmaydi.'});return;}
    const rows=await require('../database').prepare('SELECT id,owner_id,bot_id,username,first_name,status,created_at,updated_at FROM clones ORDER BY id DESC').all();await bot.answerCallbackQuery(q.id);
    return bot.sendMessage(q.message.chat.id,`🧬 CLONE BOTS\nCredits: ${await clones.credits(owner())}\n${rows.map(c=>`#${c.id} @${c.username||c.bot_id} — ${c.status}`).join('\n')||'Clone yo‘q.'}`,{reply_markup:clonesKeyboard(rows)});
   }
   if(d.startsWith('clone:stop:')){
    ...
    const id=Number(d.split(':')[2]),r=await clones.get(id);if(!r)return bot.answerCallbackQuery(q.id,{text:'Topilmadi'});runner.stop(id);await bot.answerCallbackQuery(q.id,{text:'Clone to‘xtatildi'});return bot.sendMessage(q.message.chat.id,`⏹ Clone #${id} to‘xtatildi.`);
   }
   if(d.startsWith('clone:start:')){
    if(!canManageClones()){await bot.answerCallbackQuery(q.id,{text:'Faqat asosiy owner boshqaradi.'});return;}
    const id=Number(d.split(':')[2]),r=await clones.get(id);if(!r)return bot.answerCallbackQuery(q.id,{text:'Topilmadi'});await runner.start(r);await bot.answerCallbackQuery(q.id,{text:'Clone ishga tushdi'});return bot.sendMessage(q.message.chat.id,`▶️ Clone #${id} ishga tushdi.`);
   }
   if(d.startsWith('clone:remove:')){
    if(!canManageClones()){await bot.answerCallbackQuery(q.id,{text:'Faqat asosiy owner boshqaradi.'});return;}
    const id=Number(d.split(':')[2]),r=await clones.get(id);if(!r)return bot.answerCallbackQuery(q.id,{text:'Topilmadi'});runner.stop(id);await clones.remove(id);await bot.answerCallbackQuery(q.id,{text:'Clone o‘chirildi'});return bot.sendMessage(q.message.chat.id,`🗑 Clone #${id} o‘chirildi.`);
   }
   if(d.startsWith('clone:view:')){
    if(!canManageClones()){await bot.answerCallbackQuery(q.id,{text:'Faqat asosiy owner boshqaradi.'});return;}
    const id=Number(d.split(':')[2]),r=await clones.get(id);if(!r)return bot.answerCallbackQuery(q.id,{text:'Topilmadi'});
    const s=await clones.stats(id);await bot.answerCallbackQuery(q.id);return bot.sendMessage(q.message.chat.id,`🧬 CLONE #${id}\n@${r.username||r.bot_id}\nStatus: ${r.status}\nEvents: ${s.events||0}\nUsers: ${s.users||0}\nChats: ${s.chats||0}\nGames: ${s.games||0}`,{reply_markup:cloneActions(id)});
   }
   if(d==='clone:create'){
    if(!canManageClones()){await bot.answerCallbackQuery(q.id,{text:'Clone faqat asosiy botda yaratiladi.'});return;}
    if((await clones.credits(owner()))<1){await bot.answerCallbackQuery(q.id,{text:'Clone krediti yo‘q'});return bot.sendMessage(q.message.chat.id,`❌ Avval ${config.monetization.clonePriceStars} Stars ga clone oling.`);}
