...
function register({bot,ownerId=config.ownerId,isClone=false}){
 ...
 bot.onText(/^\/progroups$/,async msg=>{if(!owner(msg.from.id,ownerId))return;const rows=await tools.listProGroups();await bot.sendMessage(msg.chat.id,'🏢 PRO GROUPS\n\n'+(rows.map(g=>`#${g.id} ${g.title} — ${g.price} ${g.currency}`).join('\n')||'Guruh yo‘q.'));});
 bot.onText(/^\/progroup\s+(-?\d+)\s+(.+?)\s+(\d+)\s+(coin|money|diamond)\s*(.*)$/i,async msg=>{if(!owner(msg.from.id,ownerId))return;const g=await tools.addProGroup(msg.match[1],msg.match[2],msg.match[3],msg.match[4].toLowerCase(),msg.match[5]||'');await bot.sendMessage(msg.chat.id,`✅ PRO Group qo‘shildi.\n🏢 ${g.title}\n💰 ${g.price} ${g.currency}\n🔗 ${g.invite_link||'link kiritilmagan'}`);});
 bot.onText(/^\/giveaway\s+(-?\d+)\s+(.+?)\s+(money|coin|diamond)\s+(\d+)\s+(\d+)$/i,async msg=>{
  ...
  const g=await tools.createGiveaway(msg.from.id,target,title,type,amount,Math.min(winners,amount));
  try{await bot.sendMessage(target,`🎁 KONKURS\n\n${title}\n🏆 Sovrin: ${amount} ${type}\n👥 G‘oliblar: ${Math.min(winners,amount)}\n\nIshtirok etish uchun tugmani bosing.`,{reply_markup:{inline_keyboard:[[{text:'🎁 Ishtirok etish',callback_data:`giveaway:join:${g.id}`}]]}});return bot.sendMessage(msg.chat.id,`✅ Giveaway #${g.id} yuborildi.`);}catch(e){await tools.finishGiveaway(g.id);return bot.sendMessage(msg.chat.id,`❌ Xabar yuborilmadi: ${e.message}`);}
 });
 bot.onText(/^\/finishgiveaway\s+(\d+)$/i,async msg=>{
  if(!owner(msg.from.id,ownerId))return;
  const id=Number(msg.match[1]),g=await tools.getGiveaway(id);if(!g||Number(g.owner_id)!==Number(ownerId))return bot.sendMessage(msg.chat.id,'❌ Giveaway topilmadi.');
  if(g.status!=='active')return bot.sendMessage(msg.chat.id,'ℹ️ Giveaway allaqachon tugagan.');
  const list=await tools.entries(id);if(!list.length){await tools.finishGiveaway(id);return bot.sendMessage(msg.chat.id,'ℹ️ Ishtirokchi bo‘lmadi.');}
  ...
  for(let i=0;i<selected.length;i++){const amount=base+(i<rem?1:0);if(g.prize_type==='diamond')await users.addDiamonds(selected[i].user_id,amount);else if(g.prize_type==='coin')await users.addCoins(selected[i].user_id,amount);else await users.addMoney(selected[i].user_id,amount);try{await bot.sendMessage(selected[i].user_id,`🎉 Giveaway #${id} g‘olibi bo‘ldingiz!\n🎁 ${amount} ${g.prize_type}`);}catch(_){} }
  await tools.finishGiveaway(id);await bot.sendMessage(msg.chat.id,`🏁 Giveaway #${id} tugadi.\n👥 Ishtirokchilar: ${list.length}\n🏆 G‘oliblar: ${selected.length}\n🎁 Jami: ${g.prize_amount} ${g.prize_type}`);
 });
 bot.on('message',async msg=>{
  ...
  if(st.type==='adminmsg'&&owner(msg.from.id,ownerId)){
   ...
   for(const a of await admins.all(ownerId)){try{await bot.sendMessage(a.user_id,`📣 OWNER XABARI\n\n${msg.text}`);ok++;}catch(e){fail++;}}
 ...
 bot.on('callback_query',async q=>{
  ...
  const id=Number(d.split(':')[2]);const g=await tools.getGiveaway(id);if(!g||g.status!=='active')return bot.answerCallbackQuery(q.id,{text:'Konkurs tugagan'});
  try{const added=await tools.enterGiveaway(id,q.from);await bot.answerCallbackQuery(q.id,{text:added?'✅ Ishtirok etdingiz!':'ℹ️ Siz allaqachon ishtirokchisiz.'});}catch(e){await bot.answerCallbackQuery(q.id,{text:'Xatolik'});}
 });
...
🏁 /finishgiveaway ID — random g‘oliblarni tanlash\n\n💱 /setprice money N\n🪙 /setprice coin N\n💎 /setprice diamond N\n🧬 /setprice clone_price N`);}
   if(d==='owner:progroups'){await bot.answerCallbackQuery(q.id);const rows=await tools.listProGroups();return bot.sendMessage(q.message.chat.id,'🏢 PRO GROUPS\n\n'+(rows.map(g=>`🔹 ${g.title}\n💰 ${g.price} ${g.currency}\n🔗 ${g.invite_link||'link yo‘q'}`).join('\n\n')||'Guruh yo‘q.'));}
   ...
   if(d==='owner:giveaways'){await bot.answerCallbackQuery(q.id);const rows=await tools.activeGiveaways();return bot.sendMessage(q.message.chat.id,'🏆 ACTIVE GIVEAWAYS\n\n'+(rows.map(g=>`#${g.id} ${g.title} — ${g.prize_amount} ${g.prize_type}, ${g.winners_count} winners`).join('\n')||'Faol konkurs yo‘q.')+'\n\nYangi: /giveaway CHAT_ID TITLE diamond 200 200\nTugatish: /finishgiveaway ID');}
