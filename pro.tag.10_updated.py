...
class UserStatesGroup(StatesGroup):
    ...
    module_toggle          = State()
    owner_add_admin         = State()
    owner_remove_admin      = State()
    user_market_add_word    = State()
    owner_sticker_add       = State()
...
async def init_db():
    async with aiosqlite.connect(DB_FILE) as db:
        ...
        # Per-owner feature switches.
        await db.execute("""
        CREATE TABLE IF NOT EXISTS owner_modules (
            owner_id TEXT NOT NULL,
        ...
        # Groups selected by the account owner for UTag.
        await db.execute("""
        CREATE TABLE IF NOT EXISTS utag_groups (
            owner_id TEXT NOT NULL,
...
# ─────────────────────────────────────────────
# OWNER / MODULE / UTAG GROUP HELPERS
# ─────────────────────────────────────────────
OWNER_MODULES = {
    "raid": "Reyd",
    "pm": "PM",
...
async def is_owner(uid: str) -> bool:
    return int(uid) == ADMIN_ID

async def is_bot_admin(uid: str) -> bool:
    if int(uid) in ADMIN_IDS:
        return True
    async with aiosqlite.connect(DB_FILE) as db:
        async with db.execute(
            "SELECT 1 FROM bot_admins WHERE owner_id=? AND admin_id=?",
            (str(ADMIN_ID), str(uid))
        ) as cur:
            return await cur.fetchone() is not None

async def owner_module_enabled(uid: str, module: str) -> bool:
    if not await is_owner(uid):
        # Existing hard-coded admins remain privileged.
        return True
    async with aiosqlite.connect(DB_FILE) as db:
        async with db.execute(
            "SELECT enabled FROM owner_modules WHERE owner_id=? AND module=?",
            (str(ADMIN_ID), module)
        ) as cur:
            row = await cur.fetchone()
    return bool(row[0]) if row else True

async def ensure_owner_modules():
    async with aiosqlite.connect(DB_FILE) as db:
        for module in OWNER_MODULES:
            await db.execute(
                "INSERT OR IGNORE INTO owner_modules(owner_id,module,enabled,updated_at) VALUES(?,?,1,?)",
                (str(ADMIN_ID), module, datetime.now(timezone.utc).isoformat())
            )
        await db.commit()

async def get_selected_utag_groups(uid: str):
    async with aiosqlite.connect(DB_FILE) as db:
        async with db.execute(
            "SELECT chat_id,title,username FROM utag_groups WHERE owner_id=? ORDER BY title",
            (uid,)
        ) as cur:
            return await cur.fetchall()

async def save_selected_utag_group(uid: str, chat_id: int, title: str, username: str | None):
    async with aiosqlite.connect(DB_FILE) as db:
        await db.execute(
            "INSERT OR REPLACE INTO utag_groups(owner_id,chat_id,title,username) VALUES(?,?,?,?)",
            (uid, str(chat_id), title[:200], username)
        )
        await db.commit()

async def remove_selected_utag_group(uid: str, chat_id: str):
    async with aiosqlite.connect(DB_FILE) as db:
        await db.execute(
            "DELETE FROM utag_groups WHERE owner_id=? AND chat_id=?",
            (uid, str(chat_id))
        )
        await db.commit()

async def get_opt_in_users(client: TelegramClient, chat):
    """UTag uchun faqat o'z xohishi bilan opt-in qilgan a'zolarni qaytaradi."""
    async with aiosqlite.connect(DB_FILE) as db:
        async with db.execute(
            "SELECT user_id FROM group_members_activity WHERE chat_id=? AND opt_in=1",
            (str(getattr(chat, "id", "")),)
        ) as cur:
    ...
    async for participant in client.iter_participants(chat, limit=None):
        if (
            ...
            and not getattr(participant, "deleted", False)
        ):
            result.append(participant)
    return result

def owner_profile_keyboard():
    kb = InlineKeyboardBuilder()
    kb.row(
        InlineKeyboardButton(text="👤 Admin qo'shish", callback_data="owner_add_admin"),
        InlineKeyboardButton(text="🗑 Admin o'chirish", callback_data="owner_remove_admin"),
    )
...
async def owner_modules_keyboard():
    kb = InlineKeyboardBuilder()
    async with aiosqlite.connect(DB_FILE) as db:
        async with db.execute(
            "SELECT module,enabled FROM owner_modules WHERE owner_id=? ORDER BY module",
            (str(ADMIN_ID),)
        ) as cur:
            rows = await cur.fetchall()
    for module, enabled in rows:
        label = OWNER_MODULES.get(module, module)
        kb.row(InlineKeyboardButton(
            text=("🟢 " if enabled else "🔴 ") + label,
            callback_data=f"owner_mod:{module}"
        ))
    ...
    for chat_id, title, username in groups:
        kb.row(
            InlineKeyboardButton(
                text=f"▶️ {title[:30]}",
                callback_data=f"utag_group_start:{chat_id}"
            ),
            InlineKeyboardButton(
                text="🗑",
...
async def cmd_start(message: Message, state: FSMContext):
    ...
    # /start dan keyin akkaunt ulangan bo'lmasa, ulashni birinchi o'ringa chiqaramiz.
    existing_session = False
    async with aiosqlite.connect(DB_FILE) as db:
        async with db.execute(
            "SELECT 1 FROM user_sessions WHERE user_id=? LIMIT 1", (uid,)
        ) as cur:
            existing_session = await cur.fetchone() is not None

    if not existing_session:
        kb = InlineKeyboardBuilder()
        kb.row(InlineKeyboardButton(text="📱 Akkauntni ulash", callback_data="add_main_account"))
        kb.row(InlineKeyboardButton(text="➡️ Keyinroq", callback_data="main_menu"))
        await message.answer(
            "🤖 <b>Auto akkaunt ulash</b>\n\n"
            "uTag va guruh funksiyalaridan foydalanish uchun Telegram akkauntingizni ulashing.\n"
            "Tasdiqlash kodi va 2FA faqat Telegramning o'z login jarayonida ishlatiladi.",
...
async def cb_how_utag(callback: CallbackQuery):
    text = (
        ...
        "2️⃣ Owner profildan <b>UTag guruhlarini tanlash</b> bo'limiga kiring\n"
        "3️⃣ 🔎 Admin bo'lgan guruhlarni toping va ▶️ tugma bilan tanlangan "
        "guruhda UTagni boshlang\n"
        "4️⃣ Guruh ichida <code>.u</code> yoki <code>.su</code> yozsangiz ham "
        "UTag boshlanadi\n"
        "5️⃣ Kulgili random UTag uchun <code>.ru</code> yozing\n"
        "6️⃣ Guruhda <code>.f</code> yoki <code>.p</code> yozsangiz UTag "
        "to'xtatiladi\n\n"
...
async def do_utag(
    ...
    """Tanlangan/ admin bo'lgan guruhda faqat opt-in qilgan a'zolarni UTag qiladi.
    ...
    selected_ids = {str(g[0]) for g in groups}

    # UTag faqat akkaunt admin bo'lgan yoki oldindan tanlangan guruhlarda ishlaydi.
    is_admin = False
    try:
        ...
        is_admin = False
    if chat_id not in selected_ids and not is_admin:
        return

    chat_title = getattr(chat, "title", str(getattr(chat, "id", "")))
    pro_until = None
    async with aiosqlite.connect(DB_FILE) as db:
        async with db.execute("SELECT pro_until FROM users WHERE id = ?", (uid,)) as cur:
            row = await cur.fetchone()
        pro_until = row[0] if row else None
    is_pro = is_pro_user(pro_until)
    ...
    try:
        ...
        if random_mode and not market_words:
            await client.send_message(
                chat,
                "❌ .ru uchun so'z tanlanmagan. Botdagi 🛒 So'zlar Marketidan tanlang."
            )
            return
        ...
        for user in participants:
            ...
            try:
                ...
                if tag_entity:
                    await client.send_message(chat, tag_text, formatting_entities=[tag_entity])
                else:
...
async def register_userbot_handlers(client: TelegramClient, uid: str):
    ...
    @client.on(events.NewMessage(pattern=r'^[./](su|u)$', incoming=False, outgoing=True))
    async def on_start_utag(event):
        """.u / .su — shu guruhda UTagni boshlaydi (faqat admin bo'lgan guruhlarda)."""
        ...
        task = asyncio.create_task(do_utag(client, uid, chat, random_mode=False))
    ...
    async def on_start_random_utag(event):
        """.ru — har bir userga random so'z bilan uTag (faqat admin bo'lgan guruhlarda)."""
        if event.chat_id is None:
            return
        try:
            chat = await event.get_chat()
        ...
        if uid in _utag_tasks and not _utag_tasks[uid].done():
            _utag_tasks[uid].cancel()
            await asyncio.sleep(0.5)
        task = asyncio.create_task(do_utag(client, uid, chat, random_mode=True))
        _utag_tasks[uid] = task
        try:
            await event.delete()
        except Exception:
    ...
    @client.on(events.NewMessage(pattern=r'^[./](p)$', incoming=False, outgoing=True))
    async def on_pause_utag(event):
        """UTagni to'xtatadi; qayta .u/.ru bilan davom ettirish mumkin."""
        task = _utag_tasks.get(uid)
        if task and not task.done():
            task.cancel()
        try:
            await event.delete()
    ...
    async def on_stop_utag(event):
        """.f — aktiv UTagni butunlay to'xtatadi."""
        if uid in _utag_tasks and not _utag_tasks[uid].done():
            _utag_tasks[uid].cancel()
        try:
            await event.delete()
...
async def scrape_count_input(message: Message, state: FSMContext):
    ...
    try:
        ...
        # Adminlar alohida ajratiladi. Bu ma'lumot faqat natijani
        # guruhlash uchun ishlatiladi; avtomatik admin qo'shish qilinmaydi.
        try:
            async for admin_user in client.iter_participants(
                group, filter=ChannelParticipantsAdmins(), limit=None
            ):
                if getattr(admin_user, "id", None):
                    admin_ids.add(int(admin_user.id))
        except Exception as exc:
            log.warning(f"Adminlar ro'yxatini olish cheklangan ({uid}): {exc}")
        ...
        for username, fullname, user_id in found.values():
            ...
            line = f"{display} | {safe_username} | ID: <code>{user_id}</code>"
            if int(user_id) in admin_ids:
                admin_lines.append(line)
            else:
                member_lines.append(line)

        user_lines = admin_lines + member_lines

        header = (
            f"👥 <b>{html.escape(group_title)}</b> dan yig'ilgan foydalanuvchilar\n"
            f"📊 Jami: <b>{len(user_lines)}</b> ta\n"
            f"👮 Adminlar: <b>{len(admin_lines)}</b> ta | "
            f"👤 Oddiy a'zolar: <b>{len(member_lines)}</b> ta\n\n"
        )
        ...
        if not user_lines:
            ...
            if admin_lines:
                chunk += "👮 <b>Adminlar</b>\n"
                for i, user_line in enumerate(admin_lines, 1):
                    line = f"{i}. {user_line}\n"
            ...
            if member_lines:
                chunk += "\n👤 <b>Oddiy a'zolar</b>\n"
            for i, user_line in enumerate(member_lines, 1):
                line = f"{i}. {user_line}\n"
                if len(chunk) + len(line) > 3900:
                    await message.answer(chunk)
                    chunk = ""
...
# OWNER PROFILE
# ─────────────────────────────────────────────
@dp.callback_query(F.data == "owner_profile")
async def cb_owner_profile(callback: CallbackQuery):
    if not await is_owner(str(callback.from_user.id)):
        await callback.answer("🔒 Faqat owner.", show_alert=True)
        return
    await callback.message.edit_text(
        "👑 <b>Owner Profil</b>\n\n"
        "Adminlar, funksiyalar va UTag guruhlarini shu yerdan boshqaring.",
        reply_markup=owner_profile_keyboard()
    )
...
async def cb_owner_add_admin(callback: CallbackQuery, state: FSMContext):
    if not await is_owner(str(callback.from_user.id)):
        return
    await state.set_state(UserStatesGroup.owner_add_admin)
    await callback.message.edit_text(
        "👤 <b>Admin qo'shish</b>\n\n"
        "Telegram user ID yuboring. Masalan: <code>123456789</code>",
        reply_markup=back_kb("owner_profile")
    )

@dp.message(StateFilter(UserStatesGroup.owner_add_admin), F.text)
async def owner_add_admin_input(message: Message, state: FSMContext):
    if not await is_owner(str(message.from_user.id)):
        return
    raw = message.text.strip()
    if not raw.isdigit():
        await message.answer("❌ Faqat raqamli Telegram user ID yuboring.")
        return
    async with aiosqlite.connect(DB_FILE) as db:
        await db.execute(
            "INSERT OR IGNORE INTO bot_admins(owner_id,admin_id,added_at) VALUES(?,?,?)",
            (str(ADMIN_ID), raw, datetime.now(timezone.utc).isoformat())
        )
        await db.commit()
    await state.clear()
    await message.answer(
        f"✅ <code>{raw}</code> admin qilindi.",
        reply_markup=owner_profile_keyboard()
    )
...
async def cb_owner_remove_admin(callback: CallbackQuery, state: FSMContext):
    ...
    async with aiosqlite.connect(DB_FILE) as db:
        async with db.execute(
            "SELECT admin_id FROM bot_admins WHERE owner_id=? ORDER BY admin_id",
            (str(ADMIN_ID),)
        ) as cur:
            rows = await cur.fetchall()
    if not rows:
    ...
    for (admin_id,) in rows:
        kb.row(InlineKeyboardButton(
            text=f"🗑 {admin_id}",
            callback_data=f"owner_del_admin:{admin_id}"
        ))
    kb.row(InlineKeyboardButton(text="⬅️ Orqaga", callback_data="owner_profile"))
    await callback.message.edit_text("🗑 <b>Adminni tanlang:</b>", reply_markup=kb.as_markup())

@dp.callback_query(F.data.startswith("owner_del_admin:"))
async def cb_owner_del_admin(callback: CallbackQuery):
    if not await is_owner(str(callback.from_user.id)):
        return
    admin_id = callback.data.split(":", 1)[1]
    async with aiosqlite.connect(DB_FILE) as db:
        await db.execute(
            "DELETE FROM bot_admins WHERE owner_id=? AND admin_id=?",
...
@dp.callback_query(F.data == "owner_modules")
async def cb_owner_modules(callback: CallbackQuery):
    if not await is_owner(str(callback.from_user.id)):
        return
    await callback.message.edit_text(
        "🎛 <b>Funksiyalar</b>\n\n"
        "Har bir modulni alohida yoqish/o'chirish mumkin.",
        reply_markup=await owner_modules_keyboard()
    )

@dp.callback_query(F.data.startswith("owner_mod:"))
async def cb_owner_mod_toggle(callback: CallbackQuery):
    if not await is_owner(str(callback.from_user.id)):
        return
    module = callback.data.split(":", 1)[1]
    if module not in OWNER_MODULES:
        return
    async with aiosqlite.connect(DB_FILE) as db:
        await db.execute(
            "UPDATE owner_modules SET enabled=CASE enabled WHEN 1 THEN 0 ELSE 1 END, updated_at=? "
            "WHERE owner_id=? AND module=?",
            (datetime.now(timezone.utc).isoformat(), str(ADMIN_ID), module)
        )
        await db.commit()
    await callback.answer("✅ Holat o'zgartirildi.")
    await callback.message.edit_reply_markup(reply_markup=await owner_modules_keyboard())

@dp.callback_query(F.data == "utag_groups")
async def cb_utag_groups(callback: CallbackQuery):
    uid = str(callback.from_user.id)
    if uid not in userbot_clients:
        await callback.answer("❗ Avval akkaunt ulang!", show_alert=True)
        return
...
async def cb_utag_groups_scan(callback: CallbackQuery):
    ...
    try:
        async for dialog in client.iter_dialogs():
            entity = dialog.entity
            if not getattr(entity, "megagroup", False) and not getattr(entity, "title", None):
                continue
            if not getattr(entity, "title", None):
                continue
            # Only groups where the connected account is an admin/creator.
            try:
                ...
                if getattr(participant, "is_admin", False) or getattr(participant, "is_creator", False):
                    found.append((str(entity.id), entity.title, getattr(entity, "username", None)))
            except Exception:
                continue
            if len(found) >= 50:
                break
        ...
        for chat_id, title, username in found:
            kb.row(
                InlineKeyboardButton(
                    text=f"➕ {title[:25]}",
                    callback_data=f"utag_group_add:{chat_id}"
                ),
                InlineKeyboardButton(
                    text="▶️ UTag",
                    callback_data=f"utag_group_start:{chat_id}"
        ...
        await callback.message.edit_text(
            ...
            "💡 UTag har bir guruhda alohida ishlaydi va faqat admin bo'lgan "
            "guruhlarda boshlanadi.",
            reply_markup=kb.as_markup()
        )
    except Exception as e:
...
async def cb_utag_group_add(callback: CallbackQuery):
    ...
    try:
        ...
        if not (getattr(perms, "is_admin", False) or getattr(perms, "is_creator", False)):
            await callback.answer("❌ Bu guruhda akkauntingiz admin emas.", show_alert=True)
            return
        await save_selected_utag_group(
            uid, int(chat_id), getattr(entity, "title", str(chat_id)),
...
@dp.callback_query(F.data.startswith("utag_group_del:"))
async def cb_utag_group_del(callback: CallbackQuery):
    uid = str(callback.from_user.id)
    chat_id = callback.data.split(":", 1)[1]
...
async def cb_utag_group_start(callback: CallbackQuery):
    ...
    chat_id = callback.data.split(":", 1)[1]
    try:
        chat_id_n = int(chat_id)
    except ValueError:
        await callback.answer("❌ Guruh ID noto'g'ri.", show_alert=True)
        return
    client = userbot_clients[uid]
    try:
        entity = await client.get_entity(chat_id_n)
        perms = await client.get_permissions(entity, "me")
        if not (getattr(perms, "is_admin", False) or getattr(perms, "is_creator", False)):
            await callback.answer("❌ Bu guruhda akkauntingiz admin emas.", show_alert=True)
            return
    ...
    # Tanlangan guruhda alohida UTag — avvalgi aktiv task bekor qilinadi.
    await save_selected_utag_group(
        uid,
        chat_id_n,
        getattr(entity, "title", str(chat_id_n)),
        getattr(entity, "username", None),
    )
    if uid in _utag_tasks and not _utag_tasks[uid].done():
        _utag_tasks[uid].cancel()
        await asyncio.sleep(0.5)
    await callback.answer(
        "▶️ UTag boshlandi! Natija guruhga yuboriladi.\n"
        "⏹ To'xtatish: guruhda .f yoki .p yozing.",
        show_alert=True,
    )
    task = asyncio.create_task(do_utag(client, uid, entity, random_mode=False))
    _utag_tasks[uid] = task
...
def get_word_market_keyboard(packs: list[tuple[int, str, str]]) -> InlineKeyboardMarkup:
    ...
    kb.row(InlineKeyboardButton(text="➕ O'zimning so'zimni qo'shish", callback_data="market_add_word"))
...
@dp.callback_query(F.data == "market_add_word")
async def cb_market_add_word(callback: CallbackQuery, state: FSMContext):
    await state.set_state(UserStatesGroup.user_market_add_word)
    await callback.message.edit_text(
        "➕ <b>O'zingizning so'zingizni qo'shing</b>\n\n"
        "Bitta yoki bir nechta so'zni yuboring. Har birini yangi qatorda yozishingiz mumkin.",
        reply_markup=back_kb("word_market")
    )

@dp.message(StateFilter(UserStatesGroup.user_market_add_word), F.text)
async def user_market_add_word_input(message: Message, state: FSMContext):
    uid = str(message.from_user.id)
    ...
    async with aiosqlite.connect(DB_FILE) as db:
        ...
        await db.commit()
    await state.clear()
    current = await get_user_words(uid)
    await message.answer(
        f"✅ {len(words)} ta so'z qo'shildi.\n📌 Jami: {len(current)} ta",
        reply_markup=get_word_market_keyboard(await get_market_packs())
    )
...
def get_admin_keyboard() -> InlineKeyboardMarkup:
    kb = InlineKeyboardBuilder()
    kb.row(InlineKeyboardButton(text="👑 Owner profil", callback_data="owner_profile"))
...
async def cb_pm_safe(callback: CallbackQuery, state: FSMContext):
    await state.clear()
    if not await ensure_module_access(callback, "pm_safe"):
        return
...
async def safe_test_menu(callback: CallbackQuery,state:FSMContext):
    await state.clear()
    if not await ensure_module_access(callback, "safe_test"):
        return
    if not safe_admin(callback.from_user.id):
...
async def safe_utag_menu(callback:CallbackQuery):
    if not await ensure_module_access(callback, "safe_utag"):
        return
    await callback.message.edit_text(
        "📣 <b>Safe UTag</b>\n\n"
...
def security_kb() -> InlineKeyboardMarkup:
    ...
    kb.row(InlineKeyboardButton(text="🛡️ Admin Anti-Abuse",callback_data="sec_abuse"))
    kb.row(InlineKeyboardButton(text="⬅️ Orqaga",callback_data="safe_ext"))
    return kb.as_markup()
...
# ─────────────────────────────────────────────
# SAFE GROUP HELP, OWNER APPROVAL & ADMIN ANTI-ABUSE
# ─────────────────────────────────────────────
# These additions deliberately do not implement unsolicited private-message
# raids, mass admin promotion, or automated group disruption.

ACCESS_MODULES = {
    "group_help": "Guruh yordamchisi (salom/xayrlashish)",
    "safe_test": "Safe Test",
    "safe_utag": "Safe UTag",
...
async def init_safe_feature_db():
    """Additive migration; existing tables and data are never removed."""
    async with aiosqlite.connect(DB_FILE) as db:
        await db.execute("""
        ...
        await db.execute("""
        CREATE TABLE IF NOT EXISTS group_help_settings (
            chat_id TEXT PRIMARY KEY,
            title TEXT,
            welcome_enabled INTEGER NOT NULL DEFAULT 0,
            goodbye_enabled INTEGER NOT NULL DEFAULT 0,
            welcome_text TEXT NOT NULL DEFAULT '👋 Xush kelibsiz, {user}!',
            goodbye_text TEXT NOT NULL DEFAULT '👋 {user} guruhni tark etdi.',
            updated_at TEXT NOT NULL
        )""")
        await db.execute("""
        CREATE TABLE IF NOT EXISTS admin_abuse_settings (
            chat_id TEXT PRIMARY KEY,
            title TEXT,
            enabled INTEGER NOT NULL DEFAULT 1,
            ban_limit INTEGER NOT NULL DEFAULT 2,
            ban_window INTEGER NOT NULL DEFAULT 60,
            kick_limit INTEGER NOT NULL DEFAULT 5,
            kick_window INTEGER NOT NULL DEFAULT 60,
            mute_limit INTEGER NOT NULL DEFAULT 5,
            mute_window INTEGER NOT NULL DEFAULT 60,
            unban_limit INTEGER NOT NULL DEFAULT 5,
            unban_window INTEGER NOT NULL DEFAULT 60,
        ...
        await db.execute("""
        CREATE TABLE IF NOT EXISTS owner_utag_stickers (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            owner_id TEXT NOT NULL,
...
def access_label(module: str) -> str:
    return ACCESS_MODULES.get(module, module)
...
async def has_module_access(user_id: int, module: str) -> bool:
    ...
    async with aiosqlite.connect(DB_FILE) as db:
        async with db.execute(
            "SELECT 1 FROM owner_access_requests "
            "WHERE requester_id=? AND module=? AND status='approved' "
...
async def create_access_request(user_id: int, module: str) -> tuple[bool, int | None]:
    """Return (created, request_id); duplicate pending requests are ignored."""
    if module not in ACCESS_MODULES:
        return False, None
    async with aiosqlite.connect(DB_FILE) as db:
        async with db.execute(
            "SELECT id FROM owner_access_requests "
            "WHERE requester_id=? AND module=? AND status='pending' "
            "ORDER BY id DESC LIMIT 1",
            (str(user_id), module),
        ) as cur:
        ...
        cur = await db.execute(
            ...
            "(requester_id,module,status,created_at) VALUES(?,?,?,?)",
            (
                str(user_id),
                module,
                "pending",
    ...
    try:
        ...
        display = user.username and f"@{user.username}" or user.full_name or str(user_id)
    except Exception:
        display = str(user_id)
    kb = InlineKeyboardBuilder()
    kb.row(
        InlineKeyboardButton(
            text="✅ Tasdiqlash",
            callback_data=f"access:approve:{request_id}",
        ),
        InlineKeyboardButton(
            text="❌ Rad etish",
            callback_data=f"access:deny:{request_id}",
        ),
    )
    try:
        await bot.send_message(
            ADMIN_ID,
            "🔐 <b>Yangi bo'lim so'rovi</b>\n\n"
            f"👤 User: {html.escape(str(display))}\n"
            f"🧩 Bo'lim: <b>{html.escape(access_label(module))}</b>\n"
            f"🆔 ID: <code>{user_id}</code>\n\n"
            "Tasdiqlaysizmi?",
            reply_markup=kb.as_markup(),
        )
    except Exception as exc:
        log.warning("Owner approval notification failed: %s", exc)
    return True, request_id


async def ensure_module_access(callback: CallbackQuery, module: str) -> bool:
    if await has_module_access(callback.from_user.id, module):
        return True
    created, _ = await create_access_request(callback.from_user.id, module)
    message = (
        "⏳ Owner tasdig'ini kuting. So'rov yuborildi."
        if created
        else "⏳ Bu bo'lim uchun owner tasdig'i hali kutilmoqda."
    )
    await callback.answer(message, show_alert=True)
    return False


async def ensure_message_module_access(message: Message, module: str) -> bool:
    if await has_module_access(message.from_user.id, module):
        return True
    created, _ = await create_access_request(message.from_user.id, module)
    await message.answer(
        "⏳ Owner tasdig'ini kuting. "
        + ("So'rov yuborildi." if created else "So'rovingiz allaqachon yuborilgan.")
    )
    return False


@dp.message(Command("request"), F.chat.type == "private")
async def request_module_access(message: Message):
    ...
    if module not in ACCESS_MODULES:
        await message.answer(
            "🧩 So'rov yuborish uchun bo'lim nomini yozing:\n"
            "<code>/request group_help</code>\n"
            "<code>/request safe_utag</code>\n"
            "<code>/request pm_safe</code>"
        )
        return
    if await has_module_access(message.from_user.id, module):
        await message.answer(f"✅ Sizda «{html.escape(access_label(module))}» bo'limi ochiq.")
        return
    created, _ = await create_access_request(message.from_user.id, module)
    await message.answer(
        "⏳ Owner tasdig'ini kuting. "
        + ("So'rov yuborildi." if created else "So'rov allaqachon kutilmoqda.")
    )


@dp.callback_query(F.data.startswith("access:"))
async def access_decision(callback: CallbackQuery):
    ...
    try:
        ...
        return
    status = "approved" if parts[1] == "approve" else "denied"
    async with aiosqlite.connect(DB_FILE) as db:
        async with db.execute(
            "SELECT requester_id, module, status FROM owner_access_requests WHERE id=?",
            (request_id,),
        ) as cur:
            row = await cur.fetchone()
        if not row or row[2] != "pending":
            ...
            return
        await db.execute(
            "UPDATE owner_access_requests SET status=?, decided_at=?, decided_by=? WHERE id=?",
            (
                status,
                datetime.now(timezone.utc).isoformat(),
        ...
        await db.commit()
    requester_id, module, _ = row
    decision_text = "✅ tasdiqlandi" if status == "approved" else "❌ rad etildi"
    await callback.answer(decision_text, show_alert=True)
    try:
        await bot.send_message(
            int(requester_id),
            f"{decision_text.capitalize()}: <b>{html.escape(access_label(module))}</b>.",
        )
    except Exception:
        pass
    try:
...
def _group_user_link(user) -> str:
    name = html.escape(getattr(user, "full_name", None) or "Foydalanuvchi")
...
async def get_group_help_settings(chat_id: int, title: str = "") -> tuple:
    async with aiosqlite.connect(DB_FILE) as db:
        await db.execute(
            "INSERT OR IGNORE INTO group_help_settings "
            "(chat_id,title,welcome_enabled,goodbye_enabled,updated_at) VALUES(?,?,?,?,?)",
            (
                str(chat_id),
                title[:200],
                0,
        ...
        await db.commit()
        async with db.execute(
            "SELECT welcome_enabled,goodbye_enabled,welcome_text,goodbye_text "
            "FROM group_help_settings WHERE chat_id=?",
            (str(chat_id),),
        ) as cur:
            return await cur.fetchone()


async def update_group_help_setting(chat_id: int, field: str, value):
    allowed = {
        "welcome_enabled",
        "goodbye_enabled",
        "welcome_text",
    ...
    async with aiosqlite.connect(DB_FILE) as db:
        await db.execute(
            f"UPDATE group_help_settings SET {field}=?, updated_at=? WHERE chat_id=?",
            (value, datetime.now(timezone.utc).isoformat(), str(chat_id)),
        )
        await db.commit()


def group_help_text(settings: tuple) -> str:
    welcome, goodbye, welcome_text, goodbye_text = settings
    return (
        "🤖 <b>Guruh yordamchisi</b>\n\n"
        f"👋 Salomlashish: {'🟢 yoqilgan' if welcome else '🔴 o‘chirilgan'}\n"
        f"👋 Xayrlashish: {'🟢 yoqilgan' if goodbye else '🔴 o‘chirilgan'}\n\n"
        "Buyruqlar:\n"
        "<code>/welcome on|off</code>\n"
        "<code>/goodbye on|off</code>\n"
        "<code>/greet Yangi matn</code>\n"
        "<code>/farewell Yangi matn</code>\n\n"
        "Matnda <code>{user}</code> ishlatilsa, user mention qilinadi."
    )


@dp.message(Command("grouphelp"), F.chat.type.in_({"group", "supergroup"}))
async def group_help_command(message: Message):
    if not await is_group_admin(message.chat.id, message.from_user.id):
        await message.answer("🔒 Bu sozlama faqat guruh adminlari uchun.")
        return
    if not await ensure_message_module_access(message, "group_help"):
        return
    settings = await get_group_help_settings(message.chat.id, message.chat.title or "")
    await message.answer(group_help_text(settings))


@dp.message(Command("welcome"), F.chat.type.in_({"group", "supergroup"}))
async def group_welcome_command(message: Message):
    if not await is_group_admin(message.chat.id, message.from_user.id):
        return
    if not await ensure_message_module_access(message, "group_help"):
        return
    ...
    value = arg[1].strip().lower() if len(arg) > 1 else ""
    if value in {"on", "off"}:
        await update_group_help_setting(message.chat.id, "welcome_enabled", int(value == "on"))
    else:
        await message.answer(group_help_text(settings))
        return
    await message.answer("✅ Salomlashish " + ("yoqildi." if value == "on" else "o‘chirildi."))


@dp.message(Command("goodbye"), F.chat.type.in_({"group", "supergroup"}))
async def group_goodbye_command(message: Message):
    if not await is_group_admin(message.chat.id, message.from_user.id):
        return
    if not await ensure_message_module_access(message, "group_help"):
        return
    await get_group_help_settings(message.chat.id, message.chat.title or "")
    arg = (message.text or "").split(maxsplit=1)
    value = arg[1].strip().lower() if len(arg) > 1 else ""
    if value not in {"on", "off"}:
        await message.answer("Foydalanish: <code>/goodbye on</code> yoki <code>/goodbye off</code>")
        return
    await update_group_help_setting(message.chat.id, "goodbye_enabled", int(value == "on"))
    await message.answer("✅ Xayrlashish " + ("yoqildi." if value == "on" else "o‘chirildi."))


@dp.message(Command("greet"), F.chat.type.in_({"group", "supergroup"}))
async def group_greet_text_command(message: Message):
    if not await is_group_admin(message.chat.id, message.from_user.id):
        return
    if not await ensure_message_module_access(message, "group_help"):
        return
    text = (message.text or "").split(maxsplit=1)
    if len(text) != 2 or not text[1].strip() or len(text[1]) > 500:
        await message.answer("Foydalanish: <code>/greet Salom, {user}!</code>")
        return
    await get_group_help_settings(message.chat.id, message.chat.title or "")
    await update_group_help_setting(message.chat.id, "welcome_text", text[1].strip())
    await message.answer("✅ Salomlashish matni saqlandi.")


@dp.message(Command("farewell"), F.chat.type.in_({"group", "supergroup"}))
async def group_farewell_text_command(message: Message):
    if not await is_group_admin(message.chat.id, message.from_user.id):
        return
    if not await ensure_message_module_access(message, "group_help"):
        return
    text = (message.text or "").split(maxsplit=1)
    if len(text) != 2 or not text[1].strip() or len(text[1]) > 500:
        await message.answer("Foydalanish: <code>/farewell Xayr, {user}!</code>")
        return
    await get_group_help_settings(message.chat.id, message.chat.title or "")
    await update_group_help_setting(message.chat.id, "goodbye_text", text[1].strip())
    await message.answer("✅ Xayrlashish matni saqlandi.")


@dp.chat_member()
async def group_member_greetings(update: ChatMemberUpdated):
    try:
        chat = update.chat
        ...
        if not user or getattr(user, "is_bot", False):
            return
        settings = await get_group_help_settings(chat.id, chat.title or "")
        if old_status in {"left", "kicked"} and new_status in {"member", "restricted"}:
            if settings[0]:
                text = settings[2].replace("{user}", _group_user_link(user))
                await bot.send_message(chat.id, text)
        elif old_status in {"member", "restricted", "administrator"} and new_status in {"left", "kicked"}:
            if settings[1]:
                text = settings[3].replace("{user}", _group_user_link(user))
                await bot.send_message(chat.id, text)
    except Exception as exc:
        log.warning("Group greeting handler failed: %s", exc)


async def get_abuse_settings(chat_id: int, title: str = "") -> tuple:
    async with aiosqlite.connect(DB_FILE) as db:
        await db.execute(
            "INSERT OR IGNORE INTO admin_abuse_settings(chat_id,title,updated_at) VALUES(?,?,?)",
            (str(chat_id), title[:200], datetime.now(timezone.utc).isoformat()),
        )
        await db.commit()
        async with db.execute(
            "SELECT enabled,ban_limit,ban_window,kick_limit,kick_window,"
            "mute_limit,mute_window,unban_limit,unban_window,punishment "
            "FROM admin_abuse_settings WHERE chat_id=?",
            (str(chat_id),),
        ) as cur:
            return await cur.fetchone()


def abuse_keyboard() -> InlineKeyboardMarkup:
    kb = InlineKeyboardBuilder()
    kb.row(
        InlineKeyboardButton(text="➖ Ban limit", callback_data="abuse:ban_dec"),
        InlineKeyboardButton(text="➕ Ban limit", callback_data="abuse:ban_inc"),
    )
    kb.row(
        InlineKeyboardButton(text="➖ Kick limit", callback_data="abuse:kick_dec"),
        InlineKeyboardButton(text="➕ Kick limit", callback_data="abuse:kick_inc"),
    )
    kb.row(
        InlineKeyboardButton(text="➖ Mute limit", callback_data="abuse:mute_dec"),
        InlineKeyboardButton(text="➕ Mute limit", callback_data="abuse:mute_inc"),
    )
    kb.row(
        InlineKeyboardButton(text="⚠️ Ogohlantirish", callback_data="abuse:punish:warn"),
        InlineKeyboardButton(text="🚪 Guruhdan chiqarish", callback_data="abuse:punish:kick"),
    )
    kb.row(InlineKeyboardButton(text="📋 Action log", callback_data="abuse:logs"))
    kb.row(InlineKeyboardButton(text="⬅️ Orqaga", callback_data="safe_security"))
    return kb.as_markup()


def abuse_text(settings: tuple) -> str:
    enabled, ban_limit, ban_window, kick_limit, kick_window, mute_limit, mute_window, unban_limit, unban_window, punishment = settings
    return (
        "🛡️ <b>Admin Anti-Abuse</b>\n\n"
        f"Holat: {'🟢 yoqilgan' if enabled else '🔴 o‘chirilgan'}\n"
        f"🔨 Ban: <b>{ban_limit}</b> ta / {ban_window} soniya\n"
        f"🚪 Kick: <b>{kick_limit}</b> ta / {kick_window} soniya\n"
        f"🔇 Mute: <b>{mute_limit}</b> ta / {mute_window} soniya\n"
        f"♻️ Unban: <b>{unban_limit}</b> ta / {unban_window} soniya\n"
        f"Jazo: <b>{'ogohlantirish' if punishment == 'warn' else 'guruhdan chiqarish'}</b>\n\n"
        "Ban limiti oshsa bot hodisani loglaydi va sozlamaga ko‘ra adminni chiqaradi."
    )


@dp.message(Command("antiabuse"), F.chat.type.in_({"group", "supergroup"}))
async def antiabuse_command(message: Message):
    if not await is_group_admin(message.chat.id, message.from_user.id):
        await message.answer("🔒 Bu sozlama guruh adminlari uchun.")
        return
    if not await ensure_message_module_access(message, "group_help"):
        return
    settings = await get_abuse_settings(message.chat.id, message.chat.title or "")
    await message.answer(abuse_text(settings), reply_markup=abuse_keyboard())


@dp.callback_query(F.data == "sec_abuse")
async def security_antiabuse_from_menu(callback: CallbackQuery):
    if callback.message.chat.type not in {"group", "supergroup"}:
        await callback.answer("Anti-Abuse panelini guruh ichida /antiabuse bilan oching.", show_alert=True)
        return
    if not await is_group_admin(callback.message.chat.id, callback.from_user.id):
        await callback.answer("🔒 Faqat guruh admini.", show_alert=True)
        return
    if not await ensure_module_access(callback, "group_help"):
        return
    settings = await get_abuse_settings(callback.message.chat.id, callback.message.chat.title or "")
    await callback.message.edit_text(abuse_text(settings), reply_markup=abuse_keyboard())


@dp.callback_query(F.data.startswith("abuse:"))
async def antiabuse_callback(callback: CallbackQuery):
    if callback.message.chat.type not in {"group", "supergroup"}:
        await callback.answer("Bu panel guruh ichida ishlaydi.", show_alert=True)
        return
    if not await is_group_admin(callback.message.chat.id, callback.from_user.id):
        await callback.answer("🔒 Faqat guruh admini.", show_alert=True)
        return
    chat_id = callback.message.chat.id
    if not await has_module_access(callback.from_user.id, "group_help"):
        await callback.answer("⏳ Owner tasdig'ini kuting.", show_alert=True)
        return
    action = callback.data.split(":")
    settings = await get_abuse_settings(chat_id, callback.message.chat.title or "")
    if action[1] == "logs":
        async with aiosqlite.connect(DB_FILE) as db:
            async with db.execute(
                "SELECT admin_username,action,target_username,reason,consequence,created_at "
                "FROM admin_action_log WHERE chat_id=? ORDER BY id DESC LIMIT 15",
                (str(chat_id),),
            ) as cur:
                rows = await cur.fetchall()
        if not rows:
            ...
            lines = ["📋 <b>Action log</b>\n"]
            for admin, act, target, reason, consequence, created in rows:
                lines.append(
                    f"• @{html.escape(admin or '—')} → {html.escape(act)} → "
                    f"@{html.escape(target or '—')}\n"
                    f"  {html.escape(reason or '—')} | {html.escape(consequence or '—')} | {html.escape(created[:19])}"
                )
            text = "\n".join(lines)
        await callback.message.edit_text(text, reply_markup=abuse_keyboard())
        return
    if action[1] == "punish" and len(action) == 3:
        punishment = action[2] if action[2] in {"warn", "kick"} else "kick"
        async with aiosqlite.connect(DB_FILE) as db:
            await db.execute(
                "UPDATE admin_abuse_settings SET punishment=?,updated_at=? WHERE chat_id=?",
                (punishment, datetime.now(timezone.utc).isoformat(), str(chat_id)),
            )
            await db.commit()
    elif action[1] in {"ban_inc", "ban_dec", "kick_inc", "kick_dec", "mute_inc", "mute_dec"}:
        field = action[1].split("_")[0] + "_limit"
        delta = 1 if action[1].endswith("_inc") else -1
        current = {"ban": settings[1], "kick": settings[3], "mute": settings[5]}[action[1].split("_")[0]]
        new_value = max(1, min(50, current + delta))
        async with aiosqlite.connect(DB_FILE) as db:
            await db.execute(
                f"UPDATE admin_abuse_settings SET {field}=?,updated_at=? WHERE chat_id=?",
                (new_value, datetime.now(timezone.utc).isoformat(), str(chat_id)),
            )
            await db.commit()
    new_settings = await get_abuse_settings(chat_id, callback.message.chat.title or "")
    await callback.answer("✅ Anti-Abuse sozlamasi yangilandi.")
    await callback.message.edit_text(abuse_text(new_settings), reply_markup=abuse_keyboard())


def _is_muted(member) -> bool:
    permissions = getattr(member, "permissions", None)
    return bool(
        getattr(member, "status", "") == "restricted"
        and permissions is not None
        and not getattr(permissions, "can_send_messages", True)
    )
...
async def log_admin_action(
    ...
    async with aiosqlite.connect(DB_FILE) as db:
        await db.execute(
            ...
            "(chat_id,chat_title,admin_id,admin_username,target_id,target_username,"
            "action,reason,consequence,created_at) VALUES(?,?,?,?,?,?,?,?,?,?)",
            (
                str(update.chat.id),
...
async def enforce_admin_abuse(update: ChatMemberUpdated, action: str):
    actor = update.from_user
    target = getattr(update.new_chat_member, "user", None)
    if not actor or not target or actor.id == target.id or actor.is_bot:
        return
    settings = await get_abuse_settings(update.chat.id, update.chat.title or "")
    enabled, ban_limit, ban_window, kick_limit, kick_window, mute_limit, mute_window, unban_limit, unban_window, punishment = settings
    if not enabled:
        return
    limits = {
        "ban": (ban_limit, ban_window),
        "kick": (kick_limit, kick_window),
        "mute": (mute_limit, mute_window),
        "unban": (unban_limit, unban_window),
    }
    ...
    async with aiosqlite.connect(DB_FILE) as db:
        async with db.execute(
            "SELECT COUNT(*) FROM admin_action_log WHERE chat_id=? AND admin_id=? "
    ...
    reason = f"{window} soniyada {limit} ta {action} limiti oshdi."
    consequence = ""
    if count >= limit:
        if punishment == "kick" and actor.id not in ADMIN_IDS:
            try:
                await bot.ban_chat_member(update.chat.id, actor.id)
                consequence = "Admin guruhdan chiqarildi"
            except Exception as exc:
                consequence = f"Jazo bajarilmadi: {str(exc)[:120]}"
        else:
            consequence = "Ogohlantirish"
        await log_admin_action(update, action, reason, consequence)
        alert = (
            "🚨 <b>ADMIN ANTI-ABUSE</b>\n"
            f"👤 Admin: {html.escape('@' + (actor.username or actor.full_name))}\n"
            f"⚠️ Sabab: {html.escape(reason)}\n"
            f"🔨 Chora: {html.escape(consequence or '—')}"
        )
        try:
            await bot.send_message(update.chat.id, alert)
        except Exception:
            pass
        try:
        ...
        await log_admin_action(update, action, "Kuzatuv", "Hech qanday chora yo‘q")


@dp.chat_member()
async def admin_action_observer(update: ChatMemberUpdated):
    old_status = getattr(update.old_chat_member, "status", "")
    ...
    if new_status == "kicked":
        action = "ban"
    elif old_status == "kicked" and new_status in {"member", "restricted", "administrator"}:
        action = "unban"
    elif old_status == "administrator" and new_status in {"member", "restricted"}:
        action = "kick" if new_status == "member" else "mute"
    elif not old_muted and new_muted:
        action = "mute"
    if action:
        try:
            await enforce_admin_abuse(update, action)
        except Exception as exc:
            log.error("Admin anti-abuse observer: %s", exc)


@dp.message(Command("utag_optin"), F.chat.type.in_({"group", "supergroup"}))
async def utag_optin_command(message: Message):
    async with aiosqlite.connect(DB_FILE) as db:
        await db.execute(
            "INSERT INTO group_members_activity "
            "(chat_id,user_id,username,fullname,opt_in,last_seen,messages) VALUES(?,?,?,?,1,?,0) "
            "ON CONFLICT(chat_id,user_id) DO UPDATE SET opt_in=1,last_seen=excluded.last_seen",
            (
                str(message.chat.id),
        ...
        await db.commit()
    await message.answer("✅ Siz bu guruhda UTag uchun opt-in qildingiz.")


@dp.message(Command("utag_optout"), F.chat.type.in_({"group", "supergroup"}))
async def utag_optout_command(message: Message):
    async with aiosqlite.connect(DB_FILE) as db:
        await db.execute(
            "UPDATE group_members_activity SET opt_in=0,last_seen=? WHERE chat_id=? AND user_id=?",
            (datetime.now(timezone.utc).isoformat(), str(message.chat.id), str(message.from_user.id)),
        )
        await db.commit()
    await message.answer("✅ Siz UTag ro‘yxatidan chiqarildingiz.")
...
def owner_utag_stickers_keyboard(stickers: list[tuple]) -> InlineKeyboardMarkup:
    kb = InlineKeyboardBuilder()
    for sticker_id, _file_id, emoji, is_premium in stickers:
        label = f"🖼 {emoji or 'sticker'}" + (" ⭐" if is_premium else "")
        kb.row(
            InlineKeyboardButton(
                text=f"🗑 {label}",
                callback_data=f"owner_sticker_del:{sticker_id}",
            )
        )
    kb.row(
        InlineKeyboardButton(
            text="➕ Sticker qo‘shish",
            callback_data="owner_sticker_add",
        )
    )
    kb.row(InlineKeyboardButton(text="⬅️ Orqaga", callback_data="owner_profile"))
    return kb.as_markup()
...
@dp.callback_query(F.data == "owner_utag_stickers")
async def owner_utag_stickers_menu(callback: CallbackQuery):
    if not await is_owner(str(callback.from_user.id)):
        await callback.answer("🔒 Faqat owner.", show_alert=True)
        return
    stickers = await get_owner_utag_stickers()
    ...
    await callback.message.edit_text(
        text,
        reply_markup=owner_utag_stickers_keyboard(stickers),
    )


@dp.callback_query(F.data == "owner_sticker_add")
async def owner_sticker_add_start(callback: CallbackQuery, state: FSMContext):
    if not await is_owner(str(callback.from_user.id)):
        await callback.answer("🔒 Faqat owner.", show_alert=True)
        return
    await state.set_state(UserStatesGroup.owner_sticker_add)
    await callback.message.edit_text(
        "🖼 <b>UTag sticker qo‘shish</b>\n\n"
        "Sticker yuboring. Premium sticker bo‘lsa, Telegram ruxsat bergan "
        "holatda premium belgisi bilan saqlanadi.",
        reply_markup=back_kb("owner_utag_stickers"),
    )


@dp.message(StateFilter(UserStatesGroup.owner_sticker_add))
async def owner_sticker_add_input(message: Message, state: FSMContext):
    if not await is_owner(str(message.from_user.id)):
        return
    sticker = message.sticker
    if not sticker:
        await message.answer("❌ Sticker yuboring.")
        return
    async with aiosqlite.connect(DB_FILE) as db:
        await db.execute(
            "INSERT OR IGNORE INTO owner_utag_stickers "
            "(owner_id,file_id,emoji,is_premium,created_at) VALUES(?,?,?,?,?)",
            (
                str(ADMIN_ID),
                sticker.file_id,
                getattr(sticker, "emoji", "") or "",
                int(bool(getattr(sticker, "is_premium", False))),
                datetime.now(timezone.utc).isoformat(),
            ),
        )
        await db.commit()
...
@dp.callback_query(F.data.startswith("owner_sticker_del:"))
async def owner_sticker_delete(callback: CallbackQuery):
    if not await is_owner(str(callback.from_user.id)):
        await callback.answer("🔒 Faqat owner.", show_alert=True)
        return
    try:
        ...
        await callback.answer("❌ Sticker ID noto‘g‘ri.", show_alert=True)
        return
    async with aiosqlite.connect(DB_FILE) as db:
        await db.execute(
            "DELETE FROM owner_utag_stickers WHERE id=? AND owner_id=?",
            (sticker_id, str(ADMIN_ID)),
        )
        await db.commit()
    stickers = await get_owner_utag_stickers()
    await callback.answer("🗑 Sticker o‘chirildi.")
    await callback.message.edit_reply_markup(
        reply_markup=owner_utag_stickers_keyboard(stickers)
    )


async def bot_full_admin_report(chat_id: int) -> tuple[bool, list[str]]:
    """Check full admin rights before group UTag starts."""
    try:
        me = await bot.get_me()
        member = await bot.get_chat_member(chat_id=chat_id, user_id=me.id)
    except Exception as exc:
        return False, [f"Bot huquqini tekshirib bo‘lmadi: {str(exc)[:120]}"]
    if getattr(member, "status", "") == "creator":
        return True, []
    if getattr(member, "status", "") != "administrator":
        return False, ["Bot guruhda administrator emas"]
    required = {
        "can_manage_chat": "Guruhni boshqarish",
        "can_delete_messages": "Xabarlarni o‘chirish",
        "can_restrict_members": "A'zolarni cheklash",
        "can_promote_members": "Adminlarni boshqarish",
        "can_change_info": "Guruh ma'lumotlarini o‘zgartirish",
        "can_invite_users": "A'zo taklif qilish",
        "can_pin_messages": "Xabar pin qilish",
        "can_manage_topics": "Mavzularni boshqarish",
    }
    missing = [
        label for attr, label in required.items()
        if getattr(member, attr, None) is False
    ]
    return not missing, missing


def bot_setup_keyboard() -> InlineKeyboardMarkup | None:
    if not bot_username:
        return None
    kb = InlineKeyboardBuilder()
    kb.row(
...
@dp.message(Command("utag_setup"), F.chat.type.in_({"group", "supergroup"}))
async def group_utag_setup(message: Message):
    if not await is_group_admin(message.chat.id, message.from_user.id):
        await message.answer("🔒 Bu tekshiruv faqat guruh adminlari uchun.")
        return
    if not await ensure_message_module_access(message, "safe_utag"):
        return
    ready, missing = await bot_full_admin_report(message.chat.id)
    if ready:
        await message.answer(
            "✅ Bot administrator va kerakli full huquqlar yoqilgan.\n"
            "Endi UTag uchun a'zolar /utag_optin orqali rozilik beradi."
        )
        return
    missing_text = "\n".join(f"• {html.escape(item)}" for item in missing)
    await message.answer(
        "⚠️ <b>Bot huquqlari yetarli emas</b>\n\n"
        "Botni guruhda administrator qiling va quyidagi huquqlarni bering:\n"
        f"{missing_text}\n\n"
        "Telegram’da admin qo‘shish oynasida «Barcha huquqlar»ni yoqing, "
        "so‘ng /utag_setup ni qayta yuboring.",
        reply_markup=bot_setup_keyboard(),
    )


@dp.my_chat_member()
async def bot_group_permission_prompt(update: ChatMemberUpdated):
    ...
    try:
        ...
        if ready:
            text = (
                "✅ Bot guruhga ulandi va full admin huquqlari yoqilgan.\n"
                "Opt-in UTag: /utag_optin | Boshlash: /utag"
            )
        else:
            text = (
                "⚠️ <b>Bot uchun full admin huquqlari kerak</b>\n\n"
                "Telegram admin qo‘shish oynasida «Barcha huquqlar»ni yoqing:\n"
                + "\n".join(f"• {html.escape(item)}" for item in missing)
                + "\n\nKeyin tekshirish: /utag_setup"
            )
        await bot.send_message(update.chat.id, text, reply_markup=bot_setup_keyboard())
    except Exception as exc:
        log.warning("Bot group permission prompt failed: %s", exc)


async def get_bot_utag_users(chat_id: int, limit: int = 20) -> list[tuple]:
    async with aiosqlite.connect(DB_FILE) as db:
        async with db.execute(
            "SELECT user_id,username,fullname FROM group_members_activity "
            "WHERE chat_id=? AND opt_in=1 ORDER BY last_seen DESC LIMIT ?",
            (str(chat_id), limit),
        ) as cur:
            return await cur.fetchall()


@dp.message(Command("utag"), F.chat.type.in_({"group", "supergroup"}))
async def group_utag_command(message: Message):
    if not await is_group_admin(message.chat.id, message.from_user.id):
        await message.answer("🔒 UTagni faqat guruh admini ishga tushira oladi.")
        return
    if not await ensure_message_module_access(message, "safe_utag"):
        return
    ready, missing = await bot_full_admin_report(message.chat.id)
    if not ready:
        await message.answer(
            "⚠️ Avval botga full admin huquqlarini bering:\n"
            + "\n".join(f"• {html.escape(item)}" for item in missing)
            + "\n\nTekshirish: /utag_setup"
        )
        return
    users = await get_bot_utag_users(message.chat.id, limit=20)
    if not users:
        await message.answer(
            "❌ Opt-in user topilmadi. Guruh a'zolari /utag_optin yuborishi kerak."
        )
        return

    command_args = (message.text or "").split(maxsplit=1)
    words = parse_market_words(command_args[1]) if len(command_args) == 2 else []
    if not words:
        words = await get_user_words(str(message.from_user.id))
    if not words and message.from_user.id != ADMIN_ID:
        words = await get_user_words(str(ADMIN_ID))
    stickers = await get_owner_utag_stickers()
    sent = 0
    for index, (user_id, username, fullname) in enumerate(users):
        display_name = html.escape(fullname or username or "Foydalanuvchi")
        mention = f'<a href="tg://user?id={int(user_id)}">{display_name}</a>'
        word = random.choice(words) if words else ""
        text = f"{mention}, {html.escape(word)}" if word else mention
        try:
            await bot.send_message(message.chat.id, text)
            if stickers:
                sticker = random.choice(stickers)
                await bot.send_sticker(message.chat.id, sticker[1])
            sent += 1
            await asyncio.sleep(2.0)
        except TelegramRetryAfter as exc:
            await message.answer(
                f"⏹️ Telegram flood-limit berdi. UTag to‘xtatildi: {sent} ta yuborildi."
            )
            log.warning("Group UTag flood-limit: %s", exc)
            break
        except Exception as exc:
            log.warning("Group UTag send failed: %s", exc)
    await message.answer(
        f"✅ Guruh UTag tugadi: <b>{sent}</b> ta opt-in user belgilandi.\n"
        f"📝 So‘zlar: {'yoqilgan' if words else 'o‘chirilgan'} | "
        f"🖼 Stickerlar: {'yoqilgan' if stickers else 'o‘chirilgan'}"
    )
...
def start_health_server():
    """Render/health-check uchun PORT da oddiy HTTP server (bot uzilmaganini ko'rsatadi)."""
    ...
    try:
        ...
        log.warning("Health server ishga tushmadi: %s", exc)
        return
